SELECT DISTINCT
    ...
    ( (SELECT percentile_disc(0.10) WITHIN GROUP (ORDER BY creationDay) AS startAnchorDate FROM creationDayNumMessages) + INTERVAL (-11 + people2Hops.person1Id % 9) DAY )::DATE AS 'startDate:DATE',
    ( (SELECT percentile_disc(0.10) WITHIN GROUP (ORDER BY creationDay) AS endAnchorDate   FROM creationDayNumMessages) + INTERVAL ( 11 + people2Hops.person1Id % 9) DAY )::DATE AS 'endDate:DATE'
FROM people2Hops
