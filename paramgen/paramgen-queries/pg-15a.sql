SELECT DISTINCT
    ...
    ( (SELECT percentile_disc(0.95) WITHIN GROUP (ORDER BY creationDay) AS startAnchorDate FROM creationDayNumMessages) - INTERVAL (q15a_core.person1Id % 7) DAY )::DATE AS 'startDate:DATE',
    ( (SELECT percentile_disc(0.95) WITHIN GROUP (ORDER BY creationDay) AS endAnchorDate   FROM creationDayNumMessages) + INTERVAL (q15a_core.person2Id % 7) DAY )::DATE AS 'endDate:DATE'
FROM q15a_core
