...
WITH Zombies AS (
    SELECT Person.id AS zombieid
      ...
      LEFT JOIN MessageThread
        ...
        AND MessageThread.creationDate BETWEEN Person.creationDate AND :endDate -- the lower bound is an optmization to prune messages
    ...
    HAVING count(MessageThread.MessageId) < 12*extract(YEAR FROM :endDate) + extract(MONTH FROM :endDate)
...
SELECT Z.zombieid AS "zombie.id"
  ...
  FROM MessageThread
       INNER JOIN Person_likes_Message ON (MessageThread.MessageId = Person_likes_Message.MessageId)
       ...
       RIGHT JOIN Zombies Z ON (Z.zombieid = MessageThread.CreatorPersonId)
 GROUP BY Z.zombieid
